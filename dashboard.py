...
tab1, tab2, tab3, tab4, tab5, tab6, tab7 = st.tabs([
    "Overview", "📊 Summary", "📈 Main Visualizations", "⏱️ Task Duration Distribution",
...
with tab3:
    ...
    fig_avg_hours = px.bar(
        ...
        color='Hours',
        color_discrete_sequence=px.colors.sequential.Viridis,  # Professional palette
        height=500
    )
    st.plotly_chart(fig_avg_hours, use_container_width=True)

    # bar graph
    st.subheader("Average Minutes per Day of the Week (Bars per User)")
    if not filtered_data.empty:
        filtered_data['Day'] = filtered_data['started_at'].dt.day_name()
        day_order = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday']
        avg_per_user = (
            filtered_data.groupby(['user_first_name', 'Day'])['minutes']
            .mean()
            .reset_index()
        )
        avg_per_user['Day'] = pd.Categorical(avg_per_user['Day'], categories=day_order, ordered=True)
        avg_per_user = avg_per_user.sort_values('Day')
        ...
        fig_multi_bar = px.bar(
            ...
            barmode='group',  # Bars grouped by user per day
            title="Average Minutes per Day of the Week by User",
            labels={'Day': 'Day of the Week', 'minutes': 'Average Minutes', 'user_first_name': 'User'},
            height=500
        )
        fig_multi_bar.update_xaxes(categoryorder='array', categoryarray=day_order)
        st.plotly_chart(fig_multi_bar, use_container_width=True)
    else:
        st.info("No data available for the user bar graph.")
        # Time Distribution Over Time
        st.subheader("Time Distribution Over Time")
    
    if not filtered_data.empty:
        # Choose a time granularity (day in this example)
        filtered_data['Date'] = filtered_data['started_at'].dt.date
    
        # Group by date and user, sum minutes
        time_dist = (
            filtered_data.groupby(['Date', 'user_first_name'])['minutes']
            .sum()
            .reset_index()
        )
        ...
        fig_time_dist = px.bar(
            ...
            color='user_first_name',
            barmode='group',
            title="Total Minutes per Day by User",
            labels={'Date': 'Date', 'minutes': 'Total Minutes', 'user_first_name': 'User'},
            height=500
        )
        st.plotly_chart(fig_time_dist, use_container_width=True)
    else:
        st.info("No data available for the time distribution chart.")
with tab4:
...
with tab5:
    ...
    if missing_cols:
        ...
        user_list = filtered_data['user_first_name'].dropna().unique()
        if len(user_list) == 0:
            st.info("No users available for drilldown with current filters.")
        elif len(user_list) > 1:
            st.warning("Multiple users are currently selected. Please filter for one user to view individual insights.")
        else:
